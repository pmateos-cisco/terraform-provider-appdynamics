...
// importCompositeID parses the "application_id/job_id" import ID format,
// same convention as the alertandrespond package's resources -- synthetic
// jobs are scoped to a business application, unlike RBAC's account-wide
// entities.
func importCompositeID(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("Expected import ID in the form <application_id>/<job_id>, got: %q", req.ID),
		)
		return
	}
