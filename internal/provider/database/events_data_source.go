...
type dbEventSummaryModel struct {
...
type eventsDataSourceModel struct {
	...
	Events         []dbEventSummaryModel `tfsdk:"events"`
}

func (d *eventsDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_database_agent_events"
}

func (d *eventsDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Queries Database Monitoring agent events (account-wide) within a time range. Like " +
			"appdynamics_events, this is a reporting/lookup query over historical data, not a list of " +
			"currently-managed config objects -- results change over time as new events occur.",
		Attributes: map[string]schema.Attribute{
			...
			"event_types": schema.ListAttribute{
				...
				Required:    true,
				Description: "Event types to include, e.g. [\"POLICY_CONTINUES_CRITICAL\"] (verified live: required -- the API rejects the request with \"event-types is not specified\" if omitted).",
			},
			"severities": schema.ListAttribute{
				ElementType: types.StringType,
				Required:    true,
				Description: "Severities to include, e.g. [\"INFO\", \"WARN\", \"ERROR\"] (verified live: required, same as event_types).",
			},
...
func (d *eventsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	found, err := database.ListEvents(ctx, d.client, params)
	if err != nil {
		resp.Diagnostics.AddError("Error Querying Database Agent Events", err.Error())
		return
	}

	events := make([]dbEventSummaryModel, 0, len(found))
	for _, e := range found {
		events = append(events, dbEventSummaryModel{
